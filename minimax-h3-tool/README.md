...
├── assets/
│   ├── demo.mp4            # Demo video (intro + live operation, 1080p)
│   └── showcase/           # 9 generated clips (each with native audio)
└── docs/
    ├── project-profile.pdf # Project profile document (Track 1 deliverable)
    └── poster.pdf          # Poster (supplementary material)
```
...
### 4. Launch the web app (Streamlit, primary)
...
(Optional alternative) Gradio UI:
...
[`assets/demo.mp4`](assets/demo.mp4) — 2:34 @ 1080p: AMD ROCm intro generated by the
model → project-name title → generated content showcase → live operation from the
web UI. All opening shots were produced by this pipeline (video + native audio).

9 additional generated clips (each with embedded stereo audio) are in
